...
> **Note:** This is a legacy guide from the original Spectre theme. The **devnose** project has customized the content schema. For current posts schema, see `src/content.config.ts` — posts no longer support `image`, `updatedAt`, or `draft` fields.
...
createdAt: 12-29-2024 # Creation date (required, format: MM-DD-YYYY or YYYY-MM-DD)
image: "../assets/spectre.png" # Optional: image for OG meta tags, referenced with relative path
...
> As for the images, it is recommended to place them in `src/content/assets`. That way, you can separate them from normal images. Images are optional in the post schema.
